...
- When AsyncAF resolves each iteration of a callback function (such as the callback to [forEach](https://async-af.js.org/AsyncAF#forEachAF), it does so in parallel or concurrently by default, and in series or sequentially by [setting a flag](https://async-af.js.org/AsyncAF#series).

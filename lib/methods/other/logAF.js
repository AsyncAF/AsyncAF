import {wrappedLog, wrappedWarn} from '../internal/logging';

/**
 * Logs items to the console (static method)
 * - if any item is a promise, logAF will first resolve it then log its value
 *
 ...
 * - to achieve in-order logging, `await` logAF:
 * ```js
 * logAF.options({ label: false, duration: false });
 * (async () => {
 *   console.log(1);
 *   await logAF(new Promise(resolve => setTimeout(() => resolve(2), 1000)));
 *   console.log(3);
 * })();
 * // 1
 * // 2
 ...
 * ```
 *
 * **experimental feature** the label may not work correctly
 ...
 * @see logAF.options(opts) to turn the label off or change its format
...
logAF.wrappedLog = wrappedLog;
logAF.wrappedWarn = wrappedWarn;

logAF.setFormat = function setFormat() {
 ...
 * ```
 *
 * @param {Object} opts - the options for logAF
 * @param {Boolean} [opts.label]
 * @param {Boolean} [opts.duration=true]
 * @param {string} [opts.labelFormat=file]
 */
/* eslint-enable */
logAF.options = function logAFOptions(opts) {
  if (typeof opts.label === 'boolean') logAF.label = opts.label;
  if (typeof opts.duration === 'boolean') logAF.duration = opts.duration;
  if (opts.labelFormat) {
    const validFormats = [
      'file',
      'path',
    ...
    const desiredFormat = opts.labelFormat.slice(0, 6);
    if (!validFormats.includes(desiredFormat)) {
      ...
      logAF.fullFormat = opts.labelFormat;
    }
  }
};

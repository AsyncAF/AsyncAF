...
const filterAF = function (callback, thisArg = undefined) {
  return this.then(arr => {
    ...
    return (this.inSeries
      ? Array.prototype.reduce.call(arr, (promise, el, i, arr) => promise.then(bools => {
        // eslint-disable-next-line no-param-reassign
        bools[i] = callback.call(thisArg, el, i, arr);
        return Promise.all(bools);
      }), Promise.all([]))
      : Promise.all(Array.prototype.map.call(arr, callback, thisArg))
    ).then(bools => {
      bools.forEach((bool, i) => { if (bool) retVal.push(arr[i]); });

...
const findAF = function (callback, thisArg = undefined) {
  return this.then(arr => {
    ...
    return this.inSeries
      ? (function seriesFindAF(i) {
        return Promise.resolve(callback.call(thisArg, filled[i], i, filled)).then(bool => {
          if (bool) return filled[i];
          if (i === filled.length - 1) return;
          return seriesFindAF(i + 1);
        });
      }(0))
      : Promise.all(filled.map(callback, thisArg)).then(bools => filled[bools.indexOf(true)]);
  });
};

...
const everyAF = function (callback, thisArg = undefined) {
  return this.then(arr => {
    ...
    const noHoles = Array.prototype.filter.call(arr, () => true);
    if (!noHoles.length) return Promise.resolve(true);
    return this.inSeries
      ? (function seriesEveryAF(i) {
        return Promise.resolve(callback.call(thisArg, noHoles[i], i, noHoles))
          .then(result => {
            if (!result) return false;
            if (i === noHoles.length - 1) return true;
            return seriesEveryAF(i + 1);
          });
      }(0))
      : Promise.all(noHoles.map(callback, thisArg)).then(bools => bools.every(Boolean));
  });
};

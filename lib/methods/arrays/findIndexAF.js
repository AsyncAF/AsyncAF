...
const findIndexAF = function (callback, thisArg = undefined) {
  return this.then(arr => {
    ...
    return this.inSeries
      ? (function seriesFindIndexAF(i) {
        return Promise.resolve(callback.call(thisArg, filled[i], i, filled)).then(bool => {
          if (bool) return i;
          if (i === filled.length - 1) return -1;
          return seriesFindIndexAF(i + 1);
        });
      }(0))
      : Promise.all(filled.map(callback, thisArg)).then(bools => bools.indexOf(true));
  });
};
